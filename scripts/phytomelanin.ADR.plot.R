...
pdf(file="suppnew.pdf", width = 15, height = 10)
par(mfrow = c(1,2))
plot(tree, cex = 0.8)
nodelabels(pie=ard.prif$lik.anc, piecol=co, cex=0.5)
tiplabels(pie=to.matrix(prif[tree$tip.label], levels(prif)),piecol=co,cex=0.2)
plot(tree, cex = 0.8)
nodelabels(pie=ard.sxyl$lik.anc, piecol=co, cex=0.5)
tiplabels(pie=to.matrix(sxyl[tree$tip.label], levels(sxyl)),piecol=co,cex=0.2)
dev.off()